...
/** Replaces the old artist "Subscription" page — same reasoning as
 * OrganizerBilling.tsx/VenueBilling.tsx: there's no real plan-tier system
 * that anything reads at request time for line-ups either, so real billing
 * is pay-per-feature (Featured placements) instead. Real PromoteCard
 * purchase flow plus real invoice history (GET /lineup/invoices, same
 * Invoice rows admin sees, filtered to this line-up's own phone), with
 * actual PDF downloads. */
export default function LineupBilling() {
  ...
  useEffect(() => {
    Promise.all([lineupApi.me(), lineupApi.invoices()])
  ...
  const download = async (inv: Invoice) => {
    ...
    try {
      await lineupApi.downloadInvoicePdf(inv.id, `${inv.number}.pdf`);
    } catch (e) {
