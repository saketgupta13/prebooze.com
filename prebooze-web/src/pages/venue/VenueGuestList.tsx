 ...
 * itself via GET/POST /venue/hosting/events/:id/guest-list. */
export default function VenueGuestList() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    setErr('');
    try {
      await venuePartner.addGuestListEntry(eventId, { name: name.trim(), phone: phone.trim(), plusOnes, companions: companions.slice(0, plusOnes) });
      setName('');
      setPhone('');
      setPlusOnes(0);
      setCompanions([]);
  ...
  const toggleArrived = async (id: string) => {
    try {
      await venuePartner.toggleGuestArrived(id);
      loadGuestList(eventId);
    } catch (e) {
  ...
  const remove = async (id: string) => {
    try {
      await venuePartner.removeGuestListEntry(id);
      loadGuestList(eventId);
    } catch (e) {
...
function PromoterGuestsSection({ eventId, events }: { eventId: string; events: Event[] }) {
  ...
  const load = () => {
    setLoading(true);
    venuePartner.promoterGuests(eventId).then(setGuests).finally(() => setLoading(false));
  };
