...
/** Venue-hosted equivalent of organizer/OrgTeamRoles.tsx — real VenueRole/
 * VenueStaff models + endpoints, scoped to this venue's Organizer panel.
 * Default roles (Owner/Manager/Door staff) are seeded server-side the first
 * time this loads for a given venue. */
export default function VenueTeamRoles() {
  const [team, setTeam] = useState<VenueStaffMember[]>([]);
  ...
  const load = () => {
    setLoading(true);
    Promise.all([venueTeam.listStaff(), venueRoles.list()])
      .then(([t, r]) => {
        setTeam(t);
        setRoles(r);
  ...
  const updateRole = async (staffId: string, roleName: string) => {
    try {
      await venueTeam.updateStaffRole(staffId, roleName);
      load();
    } catch (e) {
  ...
  const removeMember = async (id: string, name: string) => {
    ...
    try {
      await venueTeam.removeStaff(id);
      load();
    } catch (e) {
  ...
  const setPerm = async (module: string, key: OrgPermKey, value: boolean) => {
    ...
    try {
      await venueRoles.setPerm(selectedRole, module, key, value);
      setSaved(true);
  ...
  const removeRole = async () => {
    ...
    try {
      await venueRoles.remove(selectedRole);
  ...
  return (
    <div>
      ...
      <div className="tiny muted-2" style={{ marginTop: 10 }}>
        roles: Owner · Manager · Door staff — plus your custom roles · scan access is set per member on invite
      </div>
    </div>
  );
}
