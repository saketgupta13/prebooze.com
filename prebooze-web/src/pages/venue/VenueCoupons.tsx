...
/** Venue-hosted equivalent of organizer/Coupons.tsx — same promo-code CRUD,
 * scoped by Coupon.venueId to events this venue hosts itself, via
 * GET/POST /venue/hosting/coupons. */
export default function VenueCoupons() {
  ...
  const load = () => {
    Promise.all([venuePartner.coupons(), venuePartner.hostedEvents()])
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await venuePartner.upsertCoupon(payload);
      setEditingId(null);
      setCode('');
      setDescription('');
      load();
  ...
  const toggleStatus = async (c: Coupon) => {
    try {
      await venuePartner.upsertCoupon({ id: c.id, status: c.status === 'active' ? 'paused' : 'active' });
      load();
    } catch (e) {
