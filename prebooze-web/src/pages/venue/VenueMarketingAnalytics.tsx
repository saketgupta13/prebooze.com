...
export default function MarketingAnalytics() {
  ...
  useEffect(() => {
    ...
    venuePartner.marketing.analytics(eventId)
      .then(setData)
      .catch((e) => {
  ...
  if (locked) {
    return (
      <div className="stack fade" style={{ maxWidth: 640, gap: 14 }}>
        <Link to="/venue/hosting/marketing" className="link tiny" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><ArrowLeft size={14} /> Back to Marketing</Link>
        <div className="card">
          ...
          <Link to="/venue/hosting/marketing" className="btn btn-pri btn-sm" style={{ marginTop: 10, display: 'inline-block' }}>Set up marketing →</Link>
        </div>
      </div>
    );
  }
