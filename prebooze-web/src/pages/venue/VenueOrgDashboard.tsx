  ...
  useEffect(() => {
    venuePartner
      ...
      .then(async (s) => {
        ...
        // below is an all-time count, so narrowing this to live events
        // would silently drop it to 0 for any venue whose hosted events
        // have all already happened.
        const approved = evs.filter((e) => e.status === 'approved');
        const perEvent = await Promise.all(approved.map((e) => venuePartner.attendees(e.id).catch(() => [] as OrgAttendee[])));
