...
/** Venue-hosted equivalent of organizer/OrgLiveMonitor.tsx — real gate
 * monitor scoped to this venue's own hosted events, via
 * GET /venue/hosting/events/:id/live. */
export default function VenueLiveMonitor() {
  ...
  const togglePause = async () => {
    ...
    setPauseBusy(true);
    try {
      await venuePartner.setSalesPaused(eventId, !data.salesPaused);
      loadLive();
    } catch (e) {
  ...
  if (events.length === 0 && !loading) return <div className="muted small">No live events yet — the monitor works once a hosted event is approved.</div>;
