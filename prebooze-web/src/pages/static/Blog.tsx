      ...
      <div className="container">
        ...
        <Link to={`/blog/${featured.id}`} className="card" style={{ display: 'flex', gap: 22, alignItems: 'stretch', marginBottom: 26, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 280px' }}>
            <Poster hue={hueFromId(featured.id)} emoji="📰" label="cover 16:9" variant="landscape" imageUrl={featured.bannerUrl} />
          </div>
        ...
        <div className="grid-3">
          {rest.map((p) => (
            <Link key={p.id} to={`/blog/${p.id}`} className="ecard">
              <Poster hue={hueFromId(p.id)} emoji="📰" label="cover 16:9" variant="landscape" imageUrl={p.bannerUrl} />
              <div>
