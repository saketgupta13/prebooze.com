 ...
 * Scanner.tsx/Dashboard.tsx for the per-guest attendees() API this
 * deliberately doesn't touch). Same event-grouped-summary-first, live/past
...
export default function Bookings() {
  ...
  // Landing with no event picked, no search and no status filter shows the
  // event-grouped summary first — every booking ever made flattened into
  // one table isn't useful when what you actually want is "how's tonight
  // doing." Picking an event, searching, or filtering by status all fall
  // through to the same booking-list view below, just scoped differently.
  ...
  return (
    <div>
      <div className="breadcrumb">
        <Link to="/organizer/events">Events</Link> / {eventF ? (scopedEvent?.title ?? 'Bookings') : 'All events'} / Bookings
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
        <div style={{ position: 'relative', maxWidth: 280 }}>
          ...
          <input placeholder="Search booking # / name / phone" value={q} onChange={(e) => setQ(e.target.value)} style={{ width: '100%', paddingLeft: 30 }} />
        </div>
        {!showingSummary && (
        ...
        {eventF && (
          <Link to="/organizer/bookings" className="btn btn-ghost btn-sm" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
