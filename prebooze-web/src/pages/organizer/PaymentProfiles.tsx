...
/** Bank accounts an organizer can withdraw to — plural, self-serve, no admin
 * review, entirely separate from identity verification (Organizer.verified,
 * see OrganizerVerification.tsx). withdraw() always pays out to whichever
 * profile is isDefault. See OrganizerService.{list,create,update,delete,
 * setDefault}PaymentProfile. */
export default function PaymentProfiles() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editingId === 'new') await organizer.createPaymentProfile(data);
      else if (editingId) await organizer.updatePaymentProfile(editingId, data);
      setEditingId(null);
      await load();
    } catch (e) {
  ...
  const remove = async (id: string) => {
    ...
    setErr('');
    try {
      await organizer.deletePaymentProfile(id);
      await load();
    } catch (e) {
