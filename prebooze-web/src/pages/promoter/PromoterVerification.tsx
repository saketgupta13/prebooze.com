...
import { promoter, kyc, type PromoterMe } from '../../api';
 ...
 * check as organizer's individual path, minus the individual/firm split —
 * a promoter applying is always one person. Submits into the same admin
 * Verifications queue every other role's KYC already goes through — see
 * KycService.submitPromoterVerification / approve(). */
export default function PromoterVerification() {
  ...
  useEffect(() => {
    Promise.all([promoter.me(), kyc.myStatus()])
      .then(([m, submissions]) => {
        setMe(m);
        setPending(submissions.some((s) => s.kind === 'promoter' && s.status === 'pending'));
      })
  ...
  const submit = async () => {
    ...
    try {
      ...
      await kyc.submitPromoterVerification(docs);
      setDone(true);
    } catch (e) {
