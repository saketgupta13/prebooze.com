...
export const TEMPLATE_DEFS: TemplateDef[] = [
  ...
  },
  {
    ...
    defaultBody: `<p>Hey {{name}},</p><p>We'd love to have <b>{{brand}}</b> on Prebooze — WhatsApp tickets, QR entry at the door, and commission on your terms. Setting up takes a few minutes.</p>`,
    cta: { label: 'Get started →', urlTemplate: '{{webUrl}}{{onboardingPath}}' },
    tokens: ['name', 'brand', 'onboardingPath'],
  },
  {
  ...
  {
    id: 'featured_activated', name: 'Featured auto-renewal activated', category: 'Roles',
    trigger: 'A Featured placement subscription is authorized and goes live immediately (FeaturedService, subscription.activated webhook)',
    preheader: 'Your featured placement is now live',
    defaultSubject: 'You\'re featured — {{itemLabel}} is now live',
    defaultBody: `<p>Hey {{name}},</p><p><b>{{itemLabel}}</b> is now featured. It'll renew automatically each month — cancel anytime from your dashboard.</p>`,
    cta: { label: 'Open my dashboard →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'itemLabel'],
  },
  {
    id: 'featured_renewed', name: 'Featured placement renewed', category: 'Roles',
    trigger: 'A Featured placement subscription successfully auto-charges for another month (FeaturedService, subscription.charged webhook)',
    preheader: 'Your featured placement was renewed',
    defaultSubject: 'Renewed — {{itemLabel}} stays featured',
    defaultBody: `<p>Hey {{name}},</p><p>Your featured placement for <b>{{itemLabel}}</b> just renewed for another month. An invoice is attached to your billing history.</p>`,
    cta: { label: 'View billing →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'itemLabel'],
  },
  {
    id: 'featured_subscription_halted', name: 'Featured auto-renewal halted', category: 'Roles',
    trigger: 'A Featured placement subscription charge fails and Razorpay halts it (FeaturedService, subscription.halted webhook)',
    preheader: "We couldn't renew your featured placement",
    defaultSubject: "Payment failed — {{itemLabel}} is no longer featured",
    defaultBody: `<p>Hey {{name}},</p><p>We couldn't renew your featured placement for <b>{{itemLabel}}</b> — it's come down from the featured spots until this is fixed. Update your payment method to restore it.</p>`,
    cta: { label: 'Fix billing →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'itemLabel'],
  },
  {
    id: 'marketing_subscription_activated', name: 'Marketing subscription activated', category: 'Roles',
    trigger: 'A marketing subscription is authorized and goes live immediately (MarketingService, subscription.activated webhook)',
    preheader: 'Your marketing campaign is now running',
    defaultSubject: 'Your Prebooze marketing campaign for {{brand}} is live',
    defaultBody: `<p>Hey {{name}},</p><p>Your marketing subscription for <b>{{brand}}</b> is now active — it renews automatically every 30 days. Once your first campaign is set up, live performance numbers for your events will show up in your dashboard.</p>`,
    cta: { label: 'Open my dashboard →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'brand'],
  },
  {
    id: 'marketing_subscription_renewed', name: 'Marketing subscription renewed', category: 'Roles',
    trigger: 'A marketing subscription successfully auto-charges for another 30-day cycle (MarketingService, subscription.charged webhook)',
    preheader: 'Your marketing subscription was renewed',
    defaultSubject: 'Renewed — your marketing campaign for {{brand}} continues',
    defaultBody: `<p>Hey {{name}},</p><p>Your marketing subscription for <b>{{brand}}</b> just renewed for another 30 days. An invoice is attached to your billing history.</p>`,
    cta: { label: 'View billing →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'brand'],
  },
  {
    id: 'marketing_subscription_halted', name: 'Marketing auto-renewal halted', category: 'Roles',
    trigger: 'A marketing subscription charge fails and Razorpay halts it (MarketingService, subscription.halted webhook)',
    preheader: "We couldn't renew your marketing subscription",
    defaultSubject: 'Payment failed — your marketing campaign for {{brand}} has paused',
    defaultBody: `<p>Hey {{name}},</p><p>We couldn't renew your marketing subscription for <b>{{brand}}</b> — your ad campaign has paused until this is fixed. Update your payment method to restore it.</p>`,
    cta: { label: 'Fix billing →', urlTemplate: '{{webUrl}}/dashboard' },
    tokens: ['name', 'brand'],
  },
  {
