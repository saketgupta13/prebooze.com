...
export class RedirectController {
  ...
  async event(@Param('slug') slug: string) {
    ...
    return { url: `/${toCitySlug(city)}/events/${slug}`, statusCode: 301 };
  }

  @Get('venues/:id')
  @Redirect()
  async venue(@Param('id') id: string) {
    const v = await this.catalog.venue(id);
    return { url: `/${toCitySlug(v.city)}/venues/${id}`, statusCode: 301 };
  }

  @Get('organizers/:id')
  @Redirect()
  async organizer(@Param('id') id: string) {
    const o = await this.catalog.organizer(id);
    return { url: `/${toCitySlug(o.city)}/organizers/${id}`, statusCode: 301 };
  }

  @Get('promoter/:slug')
  @Redirect()
  async promoter(@Param('slug') slug: string) {
    const p = await this.catalog.promoter(slug);
    return { url: `/${toCitySlug(p.city)}/promoter/${slug}`, statusCode: 301 };
  }

  @Get('lineup/:slug')
  @Redirect()
  async lineup(@Param('slug') slug: string) {
    const l = await this.catalog.lineup(slug);
    return { url: `/${toCitySlug(l.city)}/lineup/${slug}`, statusCode: 301 };
  }
}
