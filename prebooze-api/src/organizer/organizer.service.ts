...
export class OrganizerService {
  ...
  async adminApprove(eventId: string) {
    ...
    if (owner) {
      ...
      await this.orgNotifications.notify(owner.userId, 'approved', `"${updated.title}" was approved — it's live now`, `/events/${encodeURIComponent(updated.id)}`);
    }
    return updated;
  }
  ...
  async adminReject(eventId: string, reason: string, rejectedSections: string[] = []) {
    ...
    if (owner) {
      ...
      await this.orgNotifications.notify(owner.userId, 'rejected', `"${updated.title}" was rejected${reason ? ` — ${reason}` : ''}`, `/events/${encodeURIComponent(updated.id)}`);
    }
    return updated;
  }
