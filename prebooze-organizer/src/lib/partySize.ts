/** Faithful port of prebooze-web/src/lib/partySize.ts — infers how many
 * people one unit of a ticket tier admits, straight from its own name
 * ("Couple" = 2, "Group of 5" = 5, anything else = 1). No separate field
 * to keep in sync: the organizer's own tier name is already what a guest
 * reads before buying, so it's the single source of truth. */
