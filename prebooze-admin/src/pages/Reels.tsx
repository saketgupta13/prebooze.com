...
import { VideoPicker } from '../components/ui';
  ...
  return (
    <div className="stack fade" style={{ maxWidth: 900, gap: 14 }}>
      ...
      <form className="card" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }} onSubmit={submit}>
        <VideoPicker value={videoDataUrl} onChange={setVideoDataUrl} label="⬆ upload video 9:16 · ≤50 MB" />
        ...
        <button className="btn btn-pri" style={{ height: 38 }} disabled={!videoDataUrl}>+ Publish reel</button>
      </form>
      ...
      <div className="tblwrap">
        ...
        {reels.map((r) => (
          <div key={r.id} className="trow" style={{ minWidth: 520, opacity: r.active ? 1 : 0.55 }}>
            <span style={{ width: 56 }}>
              {r.videoDataUrl ? (
                <video src={r.videoDataUrl} muted style={{ width: 40, height: 56, objectFit: 'cover', borderRadius: 6 }} />
              ) : (
